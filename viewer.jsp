<html>
<head>
<title>Apresenta��o Multim�dia</title>
...
function initMesh(data,arquivo_m3) {

mesh.type = 0;
//mesh.src.model =  "object/merces/merces_ss-n.m2.m3";
mesh.src.model = arquivo_m3;
...
window.onload = function() {
	...
	document.getElementById("divLoading").setAttribute("style" ,"display: none");
	document.getElementById("divScreen").setAttribute("style" ,"display: none");
	//adjustCanvas();
	//initMesh();
	//loadPresentation();
	//webGLStart();
...
function getfolder(e) {
	...
	for(let i = 0; i < files.length; i++){
		var arquivo = files[i];
		tipo = arquivo.name.split('.')[arquivo.name.split('.').length -1];

		if(tipo == "m3"){
			arquivo_m3 = "object/"+arquivo.webkitRelativePath+"";
		}
		if(tipo == "xml"){
			var tamanho = arquivo.webkitRelativePath.split('/').length;
			for(let i = 0; i < tamanho-1; i++){
				data += arquivo.webkitRelativePath.split('/')[i]+"/";
			}
	...
	if((data != "") && (arquivo_m3 != "") ){
		...
		var x = document.getElementById("flup");
		x.style.display = "none";

		adjustCanvas();
		initMesh(data,arquivo_m3);
		loadPresentation();
		webGLStart();
	}
