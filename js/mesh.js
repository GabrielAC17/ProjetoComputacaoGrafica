...
for (i=0, pt=0; i<numVertices; i++, pt+=3) {
	...
	if (vertices[pt] < mMin[0])
		mMin[0] = vertices[pt];
	if (vertices[pt] > mMax[0])
		mMax[0] = vertices[pt];
	
	if (vertices[pt+1] < mMin[1])
		mMin[1] = vertices[pt+1];
	if (vertices[pt+1] > mMax[1])
		mMax[1] = vertices[pt+1];
	
	if (vertices[pt+2] < mMin[2])
		mMin[2] = vertices[pt+2];
	if (vertices[pt+2] > mMax[2])
		mMax[2] = vertices[pt+2];
...
gl.bindTexture(gl.TEXTURE_2D, texture);
gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGB, width, height, 0, gl.RGB, gl.UNSIGNED_BYTE, null);//gl.RGBA
